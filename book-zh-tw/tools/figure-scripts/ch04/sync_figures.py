...
def ladder_rows(m,english=False):
    ...
    b=m['boundaries_ns']
    return [(L("SM 內柵欄",'Barrier within an SM'),b['syncthreads_one_block'],'green'),
            (L("執行緒塊簇內同步",'Within a thread\nblock cluster'),b['cluster_sync_8_blocks'],'purple'),
            (L("跨 SM 一對一交接",'One-to-one\ncross-SM handoff'),sum(b['handoff_release_acquire'])/2,'blue'),
            (L("全部 SM 匯合",'All-SM barrier'),b['all_sm_counter_barrier'][0],'blue'),
            (L("全部 SM 匯合\n並送達活化向量",'All-SM barrier\n+ vector delivery'),b['all_sm_gather_with_vector_bf16_8kib'],'blue')]
...
def draw(out,root,english=False):
    ...
    with plt.rc_context(STYLE):
        ...
        text(a,.1825,.62,L("執行緒塊簇",'Thread block cluster'),11,ha='center')
        box(a,.03,.38,.94,.12,L("片上交叉互連",'On-chip crossbar'),'orange')
        for i in range(6):box(a,.03+i*.16,.17,.14,.11,L("L2 分片",'L2 slice'),'green',11)
        text(a,.5,.12,L(f"L2 共 {m['device']['l2_mib']} MiB，每個分片負責一部分地址",f"{m['device']['l2_mib']} MiB of L2; each slice owns part of the addresses"),11,ha='center')
        box(a,.03,.02,.94,.07,L("視訊記憶體控制器與視訊記憶體",'Memory controllers and GPU memory'),'gray',11)
        arrow(a,(.4125,.68),(.4125,.50));arrow(a,(.36,.38),(.40,.28))
        arrow(a,(.44,.28),(.74,.38));arrow(a,(.7475,.50),(.7475,.68))
        text(a,.425,.54,L("① 寫入",'① Write'),11);text(a,.76,.54,L("② 讀取",'② Read'),11)
        save(f,'l2-structure')

        # One cross-SM handoff, time downward.
        f,a=canvas(5.2)
        for x,label,c in [(.22,L("生產者 SM",'Producer SM'),'blue'),(.56,L("L2 分片",'L2 slice'),'green'),(.86,L("消費者 SM",'Consumer SM'),'orange')]:
        ...
        P,S,C=.22,.56,.86
        msg(P,.82,S,.75,L("寫資料",'Write data'));msg(S,.73,P,.66,L("確認",'Ack'),False)
        a.plot([.18,.18],[.81,.67],color=COL['ink'],lw=2.2);text(a,.01,.74,L("fence：\n等待確認",'Fence:\nwait\nfor ack'),11)
        msg(P,.52,S,.45,L("寫標誌",'Write flag'))
        msg(C,.80,S,.73,L("讀標誌",'Read flag'));msg(S,.71,C,.64,L("舊值",'Old value'),False)
        msg(C,.46,S,.39,L("讀標誌",'Read flag'));msg(S,.37,C,.30,L("新值",'New value'),False)
        msg(C,.25,S,.18,L("讀資料",'Read data'));msg(S,.16,C,.09,L("資料",'Data'),False)
        a.plot([.90,.90],[.80,.30],color=COL['ink'],lw=2.2);text(a,.92,.55,L("輪詢",'Poll'),11)
        ...
        a.set(yticks=range(len(rows)),yticklabels=[r[0] for r in rows],xlim=(0,1250),xticks=range(0,1201,200),ylim=(len(rows)-.4,-.6),xlabel=L("一次同步的時間（ns）",'Time per synchronization (ns)'))
        save(f,'sync-ladder')
