...
def markdown(result):
    """Render this stage without assumptions about the generic report schema."""
    summary = result["summary"]
    tensors = result["tensors"]

    def shape(name):
        return "×".join(map(str, tensors[name]["shape"]))

    def value(item):
        return "未知 / 未提供" if item is None else f"{item:,}"

    lines = [
        "# FLUX2 Klein VAE 解码算子账",
        "",
        f"输入 `{result['latent_shape']}` → RGB `{result['output_shape']}`；"
        f"dtype={result['scenario']['dtype']}，attention={result['scenario']['attention']}。",
        "",
        f"解码权重 {summary['decoder_weight_parameters']:,} 参数 / "
        f"{summary['decoder_weight_bytes']:,} bytes；"
        f"dense matrix/conv {summary['dense_matrix_and_conv_flops']:,} FLOPs，"
        f"有效非 padding {summary['nonpadding_matrix_and_conv_flops']:,} FLOPs，"
        f"标量算术 {summary['scalar_flops']:,}。特殊函数另列。",
        "",
        f"命名张量边界峰值 {summary['declared_tensor_boundary_peak_bytes']:,} bytes，"
        f"位于 `{summary['peak_operator']}`。条件权重+边界+指定 workspace："
        f"{value(summary['conditional_weights_boundary_workspace_bytes'])} bytes。",
        "",
        "以下接口 bytes 不是 HBM 实测；边界峰值不含 primitive 内部临时量、"
        "未知 layout 复制和分配器开销。实际运行峰值未知，不能据此直接宣称设备可运行。",
        "",
        "| 步 | 算子 | 输入形状 → 输出形状 | matrix FLOPs | scalar | 特殊函数 | 读 / 写 / 权重 bytes |",
        "|---:|---|---|---:|---:|---|---|",
    ]
    for operator in result["operators"]:
        input_shapes = ", ".join(shape(name) for name in operator["inputs"])
        special = json.dumps(operator["special_ops"], ensure_ascii=False)
        lines.append(
            f"| {operator['id']} | {operator['name']} ({operator['kind']}) | "
            f"{input_shapes} → {shape(operator['output'])} | "
            f"{operator['matrix_flops']:,} | {operator['scalar_flops']:,} | {special} | "
            f"{operator['input_bytes']:,} / {operator['output_bytes']:,} / {operator['weight_bytes']:,} |"
        )
    lines += [
        "",
        "## 生命周期与复制接口",
    ...
    for name, tensor_row in tensors.items():
        lines.append(
            f"| {name} | {tensor_row['bytes']:,} | {tensor_row['producer']} | {tensor_row['last_use']} |"
        )
    lines += [
        "",
    ...
    for row in summary["upsample_contiguous_interfaces"]:
        lines.append(
            f"| {row['stage']} | {row['input_bytes']:,} | {row['triggered']} | "
            f"{value(row['materialized_copy_bytes'])} |"
        )
    lines += [
        "",
        f"持久 decode cache：{summary['persistent_decode_cache_bytes']:,} bytes；"
        f"返回后调用方输入与输出：{summary['caller_retained_input_and_output_bytes']:,} bytes。",
        "",
        "## 适用范围",
        "",
    ]
