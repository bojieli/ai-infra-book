...
def calculate(
    ...
    for report in json.loads((PROJECT / "configs/v4-optimizer-report.lock.json").read_text()):
        if hashlib.sha256((PROJECT / report["file"]).read_bytes()).hexdigest() != report["sha256"]:
            raise ValueError("V4 optimizer report evidence changed")
