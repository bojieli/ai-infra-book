...
def markdown(result):
    s = result['summary']
    lines = ["# QK—Softmax—PV：矩阵与向量交接", "", "时间为教学tick，路径与供给为声明条件，非厂商专用通路实测。每行组完成整个QK收缩后才交Softmax；槽位到PV结束才释放。", "",
             f"完成时间：{s['finish_tick']}；首次向量开工：{s['first_vector_start_tick']}；声明交接容量：{s['reserved_handoff_bytes']} bytes（通过：{s['passes_declared_handoff_capacity']}）。", "",
             f"跨单元载荷：{s['crossing_payload_bytes']} bytes；所选路径服务字节：{s['served_handoff_bytes']}。", "",
             "| 任务 | 资源 | 开始 | 结束 | 服务时间 |",
             "|---|---|---:|---:|---:|"]
    for t in result['timeline']:
        lines.append(f"| {t['id']} | {t['resource']} | {t['start_tick']} | {t['end_tick']} | {t['duration_ticks']} |")
    lines += ["", "## 槽位占用", "", "| 行组 | 槽 | 取得 | 释放 |", "|---|---:|---:|---:|"]
    for t in result['slot_ownership']:
        lines.append(f"| {t['group']} | {t['slot']} | {t['start_tick']} | {t['release_tick']} |")
    lines += ["", "## 适用范围", ""]
    lines += ['- ' + text for text in result['assumptions']]
    lines += ["", "## 完整形状、工作量与输入", "", "```json", json.dumps(result,ensure_ascii=False,indent=2), "```", ""]
    return '\n'.join(lines)
