...
def markdown(result):
    lines = ['# 图片请求的完整成片预算', '', '文件bytes与所有阶段时间由输入声明；不执行文件编解码或模型。', '']
    for row in result['variants']:
        lines += [f"## {row['variant']}", '', f"上传 {row['transmitted_input_bytes']} bytes，成片 {row['output_file_bytes']} bytes。", '',
                  '| 阶段 | 秒（精确分数） |', '| --- | ---: |']
        lines += [f"| {e['stage']} | {e['duration_seconds_exact']} |" for e in row['stages']]
        lines += ['', f"完整成片：{row['complete_final_image_seconds_exact']} 秒；复用连接：{row['reused_connection_final_seconds_exact']} 秒。预览时间未知。", '']
    lines += ['## 条件与边界', '', *['- '+x for x in result['scope']], '', '```json', json.dumps(result,ensure_ascii=False,indent=2), '```', '']
    return '\n'.join(lines)
