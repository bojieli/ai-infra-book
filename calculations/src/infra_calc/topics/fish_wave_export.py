...
def markdown(result):
    ...
    import json
    lines += ["", "## 完整阶段与复用codec账", "", "阶段接口与codec算子分别列出；codec工作只计一次。", "", "```json",
              json.dumps({"wrapper_stages": result["wrapper_stages"], "codec_operations": result["codec_operations"]}, ensure_ascii=False, indent=2), "```", ""]
    lines += ["", "## 固定来源", ""]
    lines.extend(
