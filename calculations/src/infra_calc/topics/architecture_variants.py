...
def calculate(batch=1, history=8192, tokens=1, tp=2,
    ...
    candidates = [
        ...
        ("less_kv_more_ffn_budget_match", 36, 4096, 32, 2),
    ]
    rows = []
...
def markdown(result):
    ...
    lines = ["# 官方基线与声明架构变体", "",
             ...
             "输入：`" + json.dumps(result['scenario'], ensure_ascii=False) + "`", "",
             "| 结构 | L/H/FFN/KV头 | 参数 | 差额 | 严格等参 | 矩阵FLOPs | KV bytes/token | 每rank预算占用bytes |",
             "| --- | --- | ---: | ---: | --- | ---: | ---: | ---: |"]
    for row in result['variants']:
        c, w = row['config'], row['work']
        dims = '/'.join(str(c[k]) for k in ('num_hidden_layers', 'hidden_size', 'intermediate_size', 'num_key_value_heads'))
        lines.append(f"| {row['name']} | {dims} | {row['actual_parameters']} | {row['parameter_delta']} | {row['exact_equal_parameters']} | {w['matrix_flops']} | {w['kv_bytes_per_token_per_request']} | {w['per_rank_live_budget_bytes']} |")
    for row in result['variants']:
        lines.extend(['', '## ' + row['name'], '',
                      '工作与TP预算：`' + json.dumps({k:v for k,v in row['work'].items() if k != 'operators'}, ensure_ascii=False) + '`', '',
                      '| 算子 | 形状 | repeats | 单次矩阵/标量 | 单次权重读/激活读/写bytes | 特殊函数 |',
                      '| --- | --- | ---: | --- | --- | --- |'])
        for op in row['work']['operators']:
            lines.append(f"| {op['name']} | `{json.dumps(op['shapes'])}` | {op['repeats']} | {op['matrix_flops']}/{op['scalar_flops']} | {op['weight_read_bytes']}/{op['activation_read_bytes']}/{op['activation_write_bytes']} | `{json.dumps(op['special_ops'])}` |")
    lines.extend(['', '## 容量与通信切换条件', '',
                  '容量区间含两端；通信只覆盖声明的两类decoder collective，不代表完整请求时间。'])
    for row in result['comparisons']:
        lines.append('- `' + json.dumps(row, ensure_ascii=False) + '`')
    lines.extend(['', '## 范围', ''] + ['- ' + a for a in result['assumptions']])
    lines.extend(['', '## 固定基线来源', ''] + [f"- [{r['file']}]({r['url']}) SHA256 `{r['sha256']}`" for r in result['sources']])
    return '\n'.join(lines) + '\n'
