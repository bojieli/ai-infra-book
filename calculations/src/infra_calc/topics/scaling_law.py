...
from ..units import positive_number, positive_int
...
def _finite_api(function):
    """Public numeric APIs reject nonrepresentable outputs, including nested rows."""
        ...
        try:
            ...
            def verify(value):
                if isinstance(value, float) and not math.isfinite(value):
                    raise ValueError(f'{function.__name__} produced a nonfinite result')
            ...
            raise ValueError(f'{function.__name__}: result is outside representable numeric range') from error
    return checked


def _power_term(coefficient, value, reference, exponent):
    # Keep ordinary arithmetic stable for archived teaching cases. If a ratio or
    # intermediate power loses range, combine all factors in log space first.
    ratio = value / reference
    if ratio > 0 and math.isfinite(ratio):
        try:
            term = coefficient * ratio ** (-exponent)
            if term > 0 and math.isfinite(term):
                return term
        except OverflowError:
            pass
    return math.exp(math.log(coefficient) - exponent * (math.log(value) - math.log(reference)))
...
def loss(law, N, D):
    ...
    return law['E'] + _power_term(law['A'], N, law['N0'], law['alpha']) + _power_term(law['B'], D, law['D0'], law['beta'])
...
def fit(records, exponent_grid, N0, D0):
    ...
    for alpha,beta in exponent_grid:
        ...
        try:
            rows=[[1,_power_term(1,r['N'],N0,alpha),_power_term(1,r['D'],D0,beta)] for r in train]
            E,A,B=_linear(rows,[r['loss'] for r in train])
            if not all(math.isfinite(x) for x in (E,A,B)):
                raise ValueError('nonfinite fitted coefficients')
            # Only round a negative intercept within an explicit floating-point
            # error budget to the admitted E=0 boundary. Recompute its SSE.
            tolerance=64*math.ulp(max(r['loss'] for r in train))
            if -tolerance <= E < 0:
                E=0.0
            if E<0 or A<=0 or B<=0:
                raise ValueError('coefficients violate E>=0, A>0, B>0')
            law=dict(E=E,A=A,B=B,alpha=alpha,beta=beta,N0=N0,D0=D0)
            sse=sum((loss(law,r['N'],r['D'])-r['loss'])**2 for r in train)
            if not math.isfinite(sse): raise ValueError('nonfinite fit SSE')
        except (ValueError, OverflowError, ZeroDivisionError) as error:
            rejected.append(dict(alpha=alpha,beta=beta,reason=str(error)))
            continue
    ...
    result = dict(law=best['law'],fit_sse=best['fit_sse'],fit_bounds=bounds,
    ...
    if rejected: result['rejected_candidates']=rejected
    return result
...
def compute_optimum(law, budget_flops, k=6, N_bounds=None, D_bounds=None):
    ...
    # The dimensionless normalizer may overflow even when N and D do not.
    normalizer=k*law['N0']*law['D0']
    Q=budget_flops/normalizer if normalizer else math.inf
    ratio=a*law['A']/(b*law['B']) if b*law['B'] else math.inf
    if Q>0 and math.isfinite(Q) and ratio>0 and math.isfinite(ratio):
        try:
            N=math.exp((math.log(ratio)+b*math.log(Q))/(a+b))*law['N0']
        except OverflowError:
            N=math.inf
    else:
        N=math.inf
    if not math.isfinite(N) or N<=0:
        # Weighted logs avoid both N0*D0 and alpha*A/(beta*B).
        scale=max(a,b); wa=(a/scale)/((a/scale)+(b/scale)); wb=1-wa
        logN=(math.log(a)+math.log(law['A'])-math.log(b)-math.log(law['B']))/scale/((a/scale)+(b/scale))
        logN+=wa*math.log(law['N0'])+wb*(math.log(budget_flops)-math.log(k)-math.log(law['D0']))
        N=math.exp(logN)
    lower=0.; upper=math.inf
...
def lifecycle(law, sizes, target_loss, demand, costs, k=6):
    ...
    for N in sizes:
        number(N,'N')
        gap=target_loss-law['E']-_power_term(law['A'],N,law['N0'],law['alpha'])
        if gap<=0:
            rows.append(dict(N=N,feasible=False,D=None,total_cost=None)); continue
        D=_power_term(law['D0'],gap,law['B'],1/law['beta'])
...
def calculate(inputs=None):
    if inputs is None:
        inputs = json.loads((PROJECT / 'scenarios/scaling-law-teaching.json').read_text())
    locked = json.loads((PROJECT / 'configs/scaling-law.lock.json').read_text())
    for row in locked:
        raw = (PROJECT / row['file']).read_bytes()
        if len(raw) != row['bytes'] or hashlib.sha256(raw).hexdigest() != row['sha256']:
            raise ValueError('Scaling-law archive checksum mismatch')
    s=deepcopy(inputs)
    ...
    result = dict(schema_version=1,calculation='scaling-law',scenario=s,sources=locked,
        ...
        assumptions=[('Synthetic records are teaching inputs, not real model experiments; controls and split are explicit.' if s['data_kind'] == 'synthetic_teaching' else 'Controlled records and shared control identity are caller assertions; this calculator does not independently certify their experimental provenance.'),
...
def markdown(result):
    ...
    lines += ['', '固定来源（工具提取文本与原始PDF分开）：']
    lines += [f"- [{r['id']}]({r['url']})：{r['archive_kind']}；{r['status']}；SHA `{r['sha256']}`" for r in result['sources']]
    return '\n'.join(lines)+'\n'
