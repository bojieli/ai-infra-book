...
from .connection_window import number, integer
...
def markdown(result):
    """Show per-request delivery and ACK endpoints before the full event ledger."""
    lines = ["# 连续请求、握手与双向窗口", "",
             "自定义有限教学协议；握手图、窗口增长和固定 RTO 不代表真实 TCP/QUIC 性能。",
             "", "| 请求 | 提交 s | 连接就绪 s | 完整响应 s | 最后 ACK s | 响应耗时 s |",
             "| --- | ---: | ---: | ---: | ---: | ---: |"]
    for row in result["requests"]:
        values = [row["request_id"], *[row[key] for key in (
            "submit_seconds_exact", "connection_ready_seconds_exact",
            "complete_received_seconds_exact", "last_ack_seconds_exact", "response_seconds_exact")]]
        lines.append("| " + " | ".join(map(str, values)) + " |")
    lines += ["", "完整响应到达与 ACK 排空分别报告。连接窗口按方向共享；重置窗口不清除旧请求未确认信用，后到 ACK 仍更新对应连接。",
