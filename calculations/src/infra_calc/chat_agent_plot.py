...
from .paths import PROJECT as P
from .topics import agent_trace,trace_resource_bridge
...
DIRECTORY = P/'figures/chat-agent'


def expected_inputs():
    d=calculate()
    names={x['file'] for x in d['sources']}
    names.update('sources/trace-resource-bridge/'+x['file'] for x in d['chat_sources'])
    names.update(['configs/agent-traces.lock.json','sources/trace-resource-bridge/sources.lock.json'])
    names.update(str(x.relative_to(P)) for x in (P/'src/infra_calc').rglob('*.py'))
    names.add(str(Path(__file__).relative_to(P)))
    return names
...
def verify():
    ...
    artifacts={str((DIRECTORY/name).relative_to(P)) for name in ('figure.png','figure.svg','figure.pdf','data.json')}
    for key,expected in (('inputs',expected_inputs()),('artifacts',artifacts)):
        rows=manifest.get(key)
        if not isinstance(rows,list) or any(not isinstance(row,dict) for row in rows):
            raise ValueError('Invalid chat-agent manifest '+key)
        ...
        if any(not isinstance(name,str) for name in names) or len(names)!=len(expected) or set(names)!=expected:
            raise ValueError('Incomplete or duplicate chat-agent manifest '+key)
    for row in manifest['inputs']+manifest['artifacts']:
        if hashlib.sha256((P/row['file']).read_bytes()).hexdigest()!=row['sha256']:
            raise ValueError('Stale chat-agent figure: '+row['file'])
...
def render():
    ...
    return {'directory':str(output),**verify()}
