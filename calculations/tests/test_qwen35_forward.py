...
class CsvContractTests(unittest.TestCase):
    def test_heterogeneous_repeats_preserve_work(self):
        import csv
        import io
        ...
        self.assertEqual(len(rows), len(result["operators"]))
        for metric in ("matrix_flops", "scalar_flops", "weight_read_bytes",
                       "activation_read_bytes", "activation_write_bytes"):
            self.assertEqual(
                sum(int(row[metric]) * int(row["repeats"]) for row in rows),
                result["summary"][metric],
            )
        self.assertEqual({row["layer"] for row in rows}, {"aggregate; see repeats"})
