...
from infra_calc.topics import v4_prefix_continuation as m
...
class PrefixContinuationTests(unittest.TestCase):
    ...
    def test_result_scenario_replays(self):
        for model in ('deepseek-v4-flash', 'deepseek-v4-pro'):
            result = m.calculate(model, prefix_tokens=127, new_tokens=2)
            replay = m.calculate(**result['scenario'])
            self.assertEqual(replay, result)
            self.assertNotIn('output_head', result['scenario'])

    def test_markdown_preserves_steps_and_interfaces(self):
        result = m.calculate(prefix_tokens=127, new_tokens=2)
        report = m.markdown(result)
        for row in result['steps']:
            self.assertIn(str(row['matrix_flops_effective_attention']), report)
            self.assertIn(str(row['state_resident_after_bytes']), report)
            for field in row['known_interfaces']:
                self.assertIn(field, report)
        self.assertIn('unknown', report)
        self.assertIn('parallel_cached_chunk', report)
        self.assertIn('complete_hbm_traffic_bytes', report)
        self.assertIn('discarded_intermediate_vocabulary_heads', report)
