...
class Check(unittest.TestCase):
    def test_public_request_replay_and_existing_work(self):
        segments = [dict(kind="text", tokens=13),
                    dict(kind="image", preprocessed_height=256, preprocessed_width=384)]
        inputs = dict(images=[dict(height=256, width=384, cache_hit=True)],
                      text_tokens=13, output_tokens=4)
        baseline = request_calculate(**inputs)
        result = request_calculate(**inputs, position_segments=segments)
        self.assertEqual(result["summary"], baseline["summary"])
        self.assertEqual(result, request_calculate(**result["scenario"]))
        from infra_calc.report import markdown
        self.assertIn("rope_delta", markdown(result))
        self.assertIn("direct_model_decode_arange", markdown(result))
