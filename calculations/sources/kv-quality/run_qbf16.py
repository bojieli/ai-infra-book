...
async def main(args):
    ...
    env=dict(config=config,torch=torch.__version__,vllm=vllm.__version__,
        hashes={n:hashlib.sha256((Path(__file__).parent/n).read_bytes()).hexdigest() for n in ['run_qbf16.py','probe_qbf16.py','probe.py']},
...
if __name__=='__main__':
    ...
    args=p.parse_args()
    assert args.kv_dtype=='fp8_e4m3', 'This control requires FP8 KV'
