...
def main():
 ...
 env=os.environ.copy();cuda=str(ENV/'lib/python3.10/site-packages/nvidia/cu13');env.update(CUDA_HOME=cuda,PATH=cuda+'/bin:'+env['PATH'],TVM_FFI_CACHE_DIR=str(ROOT.parent/'09-08/jit-cache-cu13-v2'))
 ...
 try:
  ...
  for policy in ['round_robin','cache_aware','power_of_two']:
   ...
   log=(out/f'router-{policy}.log').open('x');logs.append(log);p=subprocess.Popen(cmd,stdout=log,stderr=subprocess.STDOUT,start_new_session=True);procs.append(p);commands.append(cmd);wait_ready(p,31090)
   deadline=time.monotonic()+180
   while True:
    status=http(31090,'/workers')
    with (out/f'registration-{policy}.jsonl').open('a') as f:f.write(json.dumps(status)+'\n')
    ...
    time.sleep(.2)
   for r in prompts:
    rid=f'book909-{policy}-{r["id"]}';start=time.monotonic();response=http(31090,'/generate',dict(rid=rid,text=r['text'],sampling_params=dict(temperature=0,max_new_tokens=1,ignore_eos=True)));end=time.monotonic()
    record=dict(policy=policy,prompt_id=r['id'],rid=rid,start_s=start,end_s=end,response=response);records.append(record)
    with (out/'requests.jsonl').open('a') as f:f.write(json.dumps(record)+'\n')
   (out/f'metrics-{policy}.txt').write_text(str(http(31099,'/metrics')))
