...
def main():
 ...
 try:
  ...
  order=[];rng=random.Random(909)
  for trial in range(3):
   policies=['cache_first','queue_first'];rng.shuffle(policies)
   for policy in policies:order.append(dict(trial=trial,policy=policy))
  (out/'order.json').write_text(json.dumps(order,indent=2)+'\n')
  with concurrent.futures.ThreadPoolExecutor(max_workers=2) as pool:
   for case in order:
    trial=case['trial'];policy=case['policy'];label=f'book909q-{trial}-{policy}'
    for port in ports:http(port,'/flush_cache',{})
    warm=request(ports[0],{**target,'rid':label+'-warm'})
    busy=pool.submit(request,ports[0],dict(rid=label+'-busy',text=background,sampling_params=dict(temperature=0,max_new_tokens=128,ignore_eos=True)))
    ...
    while True:
     ...
     time.sleep(.01)
    selected=ports[0] if policy=='cache_first' else min(ports,key=lambda p:state[str(p)][0]['num_reqs'])
    ...
    record=dict(**case,selected_port=selected,decision_load=state,warm=warm,target=result,background=background_result,samples=samples)
  ...
  (out/'execution.json').write_text(json.dumps(dict(commands=commands,cleanup=cleanup,source_hashes={f:hashlib.sha256((ROOT/f).read_bytes()).hexdigest() for f in ['run.py','worker-command.json','agent-prompts.json']}),indent=2)+'\n')
