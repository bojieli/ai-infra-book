...
from infra_calc.topics import dense_placement, reconfiguration
...
def calculate(model='qwen3-8b',requests=4,prompt=256,outputs=8,deadline_ms=250,
    ...
    return dict(calculation='supernode-cohort-cost-candidate',scenario=scenario,
        ...
        for requests in (1,4,8):
            for deadline in (80,250,600):
                for tag,fault,recovery,fee in [('healthy',None,0,'0'),('short',50,20,'0'),('long',50,200,'1')]:
                    key=f'{model}-n{requests}-d{deadline}-{tag}'
                    results[key]=calculate(model=model,requests=requests,deadline_ms=deadline,fault_at_ms=fault,recovery_ms=recovery,recovery_fee=fee)
    Path(__file__).with_name('result.json').write_text(json.dumps(results,ensure_ascii=False,indent=2)+'\n')
