...
def calculate(
    ...
    if batch >= 64 or any(elements * 2 > 2**31 for elements in upsample_elements):
        raise ValueError("Upsample contiguous branches require an explicit layout model")
    rows = evidence()
    ...
    result = dict(
        ...
        scope=[
            ...
            "PyTorch >=2.1 nearest BF16 path; batch<64 and every upsample input numel*2<=2**31 exclude source contiguous branches whose copies depend on strides.",
