    ...
    for result, retry in [("accept", True), ("reject", True), ("reject", False)]:
        ...
        p.update(
            ...
            packets={
                ...
                "server_flight": [1200, 1200],
                "handshake_ack": [1200],
                "client_finished": [1200],
            },
        )
        rows[f"oracle-{result}-retry-{retry}"] = p
    for result in ("accept", "reject"):
        p = json.loads(json.dumps(rows[f"oracle-{result}-retry-True"]))
        p["s2c_propagation_seconds"] = "1.5"
        rows[f"mid-packet-keys-{result}"] = p
    for ack in (False, True):
        p = example()
        p.update(
            request_payload_bytes=100, response_payload_bytes=200, server_flight_ack=ack
        )
        p["packets"]["server_flight"] = [1200] * 4
        rows[f"4800-server-flight-ack-{ack}"] = p
    return rows
