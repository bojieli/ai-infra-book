...
|---|---:|---:|---:|---|
| coarse64 | 67,708,148,736 | 20.618039 | 未知 | True |
| fine256-k16 | 67,855,997,952 | 20.728132 | 未知 | True |

令 U 为尚未计入的串行阶段耗时，细粒度严格更快的条件是：

`U_fine - U_coarse < -671959/6103515625 秒`（约 -110.093763 微秒）。等号表示并列。

共同wire字节相同，共同fabric带宽不能改变两者的差值。逻辑GEMM操作数服务与物理HBM不同；U需要覆盖所选比较中的其余差异工作，不能自动当零。必要容量通过不证明运行时可部署，给定剩余耗时也不证明模型质量相同。

满足必要容量条件后的条件最快方案：None（null表示剩余输入不足；空列表表示两者均未通过容量条件）。
...
```json
{
  ...
  "scenario": {
    ...
    "fine_flops_per_second": 100000000000000,
    ...
    "coarse_remaining_ns": null,
    "fine_remaining_ns": null
  },
  "bindings": [
    {
  ...
  "variants": [
    {
      ...
      "quality": null,
      "declared_remaining_seconds_exact": null,
      "conditional_extended_seconds_exact": null
    },
    {
      "variant": "fine256-k16",
      "ranks": [
      ...
      "serial_wire_seconds_exact": "168777/156250000",
      "conditional_subaccount_seconds_exact": "2024231673/97656250000",
  ...
  "fine_remaining_minus_coarse_must_be_less_than_seconds_exact": "-671959/6103515625",
  "conditional_capacity_eligible_fastest_variants": null,
