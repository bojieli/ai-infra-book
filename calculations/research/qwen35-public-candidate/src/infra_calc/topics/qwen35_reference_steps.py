...
def supplement(batch, tokens, history, counts, record_past=False):
    ...
    if L is not None:
        ...
        if not S and (T >= 4 or record_past):
        ...
        if not S:
            add("conv.cache_initial_zero", 45, write=2 * B * channels * 4)
    else:
        outputs = kept = None
    ...
    if not recurrent:
        ...
        if not S:
            add("chunk.initial_state_zeros", 45, write=4 * B * heads * k * v)
        add(
        ...
        add(
            "recurrent.decay_exp_interface", 45, read=4 * B * heads, write=4 * B * heads
        )
        add(
    ...
    if not S:
        add("linear.cache_recurrent_initial_zero", 45, write=4 * B * heads * k * v)
    ...
    add(
        ...
        read=8 * M * (21 + 64),
        write=8 * M * (21 + 64),
        notes="Two axis slices per sin/cos:11+10 values; concat duplicates32 frequencies to64.",
    )
    ...
    add(
        ...
        15,
        scalar=4 * 32 * delta + 32 * rect,
        special={"exp": 32 * delta, "max_comparisons": 32 * delta},
        notes="Additional rectangular softmax positions plus attention-mask addition to every score slot.",
    )
    add(
        "full.kv_cache_concat",
        15,
        read=2 * 2 * B * (S + T) * 2 * 256 if S else 0,
        write=2 * 2 * B * (S + T) * 2 * 256 if S else 0,
    ...
    add(
        ...
        45,
        read=2 * (12 * M * 64 * 128 + 12 * M * 64),
        write=2 * (8 * M * 64 * 128 + 12 * M * 64),
    ...
    return dict(
        ...
        startup=dict(
            rope_inv_freq_elements=32,
            scalar_divisions=64,
            special_ops={"power": 32},
