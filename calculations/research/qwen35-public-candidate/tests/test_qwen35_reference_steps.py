...
import math
import unittest
from infra_calc.topics.qwen35_reference_steps import supplement
...
def chunks(q, k, v, b, g, initial, C):
    ...
    for off in range(0, len(q), C):
        ...
        for x in gs:
            ...
            cum.append(a)
        decay = [
            [math.exp(cum[i] - cum[j]) if j <= i else 0.0 for j in range(C)]
            for i in range(C)
        ]
        lower = [
            [bs[i] * ks[i] * ks[j] * decay[i][j] for j in range(C)] for i in range(C)
        ]
        ...
        out.extend(
            qs[i] * math.exp(cum[i]) * state
            + sum(qs[i] * ks[j] * decay[i][j] * new[j] for j in range(C))
            for i in range(C)
        )
        state = state * math.exp(cum[-1]) + sum(
            ks[i] * math.exp(cum[-1] - cum[i]) * new[i] for i in range(C)
        )
    return out[:T], state
...
class ReferenceStepTests(unittest.TestCase):
    def test_chunk_numeric_matches_recurrent_with_tail_and_history(self):
        q = [0.2, 0.7, -0.3, 0.4, 0.5]
        k = [0.1, -0.2, 0.3, 0.4, -0.1]
        v = [0.8, 0.9, 0.2, -0.1, 0.6]
        b = [0.2, 0.6, 0.3, 0.7, 0.1]
        g = [-0.1, -0.2, -0.4, -0.2, -0.3]
        for initial in [0.0, 0.6]:
    ...
    def test_reference_conv_output_slots_hand_counts(self):
        ...
        for T, S, L, O, kept in [
            ...
            actual = next(
                x
                for x in r["steps"]
                if x["name"] == "conv.additional_padded_and_discarded_products"
            )["matrix_flops"]
    ...
    def test_router_metadata_assignment_and_hit_dependence(self):
        ...
        for hist, hit in [(hot, 10), (spread, 40)]:
            ...
            one = next(
                x
                for x in r["steps"]
                if x["name"] == "router.one_hot_initialize_scatter"
            )
            wh = next(
                x for x in r["steps"] if x["name"] == "router.where_per_active_expert"
            )
    ...
    def test_record_past_initial_cache_alias_not_last4_copy(self):
        ...
        boundary = next(
            x for x in r["steps"] if x["name"] == "conv.cache_concat_or_initial_pad"
        )
    ...
    def test_causal_mask_constructor_batch_expand(self):
        ...
        compare = next(
            x
            for x in r["steps"]
            if x["name"] == "mask.causal_compare_before_batch_expand"
        )
