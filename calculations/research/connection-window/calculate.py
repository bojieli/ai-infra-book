...
def calculate(input_bytes=30_000_000,output_bytes=5_000_000,segment_bytes=25_000,
    ...
    return dict(calculation='finite-ack-window-image-candidate',scenario=scenario,transmissions=transmissions,
...
def build_results():
    fresh=[dict(direction=d,bytes=b) for d,b in [('c2s',400),('s2c',800),('c2s',400),('s2c',800)]]
    return dict(reused=calculate(),fresh=calculate(handshake=fresh),ticket=calculate(handshake=fresh[:2]),
        receive_limited=calculate(receive_window_bytes=50_000),loss=calculate(drop_upload_packet=12),
        payload_baseline=calculate(initial_window_bytes=35_000_000,max_window_bytes=35_000_000,
            receive_window_bytes=35_000_000,data_header_bytes=0,ack_bytes=0,rto_seconds='100'))

if __name__=='__main__':
    Path(__file__).with_name('result.json').write_text(json.dumps(build_results(),indent=2)+'\n')
