...
def parser() -> argparse.ArgumentParser:
    ...
    placement = sub.add_parser('dense-placement', help='Explicit Qwen Dense TP/PP/DP shapes, KV head IDs and per-card budgets')
    ...
    capacity = sub.add_parser('capacity-scan', help='Actual Qwen shapes, declared grouped storage and KV concurrency budgets')
...
def model_list() -> list[dict]:
    ...
    base_commands = {'kimi-k3': 'k3-forward', 'deepseek-v4-flash': 'v4-forward',
                     'deepseek-v4-pro': 'v4-forward', 'deepseek-v3': 'v3-forward'}
    result = []
    for model, sources in sorted(configurations.items()):
        ...
        result.append(dict(model=model, revision=source['revision'],
            ...
            component_configs=[dict(file=row['file'],status=row['status'],revision=row['revision'])
                               for row in sources],
            forward='implemented' if dense_or_moe or model == 'deepseek-r1-distill-llama-70b' else 'pending',
