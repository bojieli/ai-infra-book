...
from infra_calc.topics.ring_collective import schedule
...
def calculate(model='qwen3-8b', gradient_dtype='FP32', algorithm='hierarchical',
    ...
    return dict(calculation='hierarchical-gradient-candidate',
...
if __name__ == '__main__':
    ...
    for dtype in ('FP32', 'BF16'):
        for algorithm in ('flat_contiguous', 'flat_interleaved', 'hierarchical'):
            for nics in (1, 2):
                results[f'{dtype}-{algorithm}-nic{nics}'] = calculate(gradient_dtype=dtype, algorithm=algorithm, nics_per_server=nics)
    Path(__file__).with_name('result.json').write_text(json.dumps(results, indent=2)+'\n')
