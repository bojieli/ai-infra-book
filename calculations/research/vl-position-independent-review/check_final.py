...
spec=importlib.util.spec_from_file_location('candidate',R/'research/vl-position-bridge/public/vl_position_bridge.py');candidate=importlib.util.module_from_spec(spec);spec.loader.exec_module(candidate)
cases=[]
for h in range(1,5):
 for w in range(1,6):
  for tail in [0,2]:
...
for segments in cases:
 ...
 ids=torch.zeros((1,len(token_types)),dtype=torch.long);tt=torch.tensor([token_types]);grid=torch.tensor(grids);pos,delta=bare.get_rope_index(ids,tt,grid);c=candidate._calculate_geometry(segments,4)
...
result={'official_cpu_position_cases':len(cases),'official_consumed_decode_comparisons':len(cases)*3,'positions_and_delta_all_match':True,'single_decode_arange_observed':aranges,'text_only_fresh_direct_model_returns_none':text_only is None,'source_sha256':hashlib.sha256(source.read_bytes()).hexdigest(),'candidate_sha256':hashlib.sha256((R/'research/vl-position-bridge/public/vl_position_bridge.py').read_bytes()).hexdigest(),'scope':'Extracted official methods execute on CPU; no weights, complete model, tokenizer or GPU execution.'};(O/'final-verification.json').write_text(json.dumps(result,indent=2)+'\n');print(result)

text_result=candidate._calculate_geometry([{"kind":"text","tokens":3}],4)
assert not text_result["summary"]["rope_delta_cached"] and text_result["summary"]["rope_delta_bytes"]==0
assert [s["name"] for s in text_result["source_steps"]]==["language_model_default_arange_add"]
image_result=candidate._calculate_geometry(cases[-1],4)
steps={s["name"]:s for s in image_result["source_steps"]}
assert steps["direct_model_decode_arange"]["materialized_output_bytes"]==24
assert steps["direct_model_decode_delta_repeat_interleave"]["materialized_output_bytes"]==24
assert steps["direct_model_decode_delta_repeat_interleave"]["operand_read_bytes"]==24
print("Both required corrections independently checked")
