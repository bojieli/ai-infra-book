...
def markdown(result):
    ...
    lines = [
        "# V4-Flash 训练：mHC 外层联合反向",
        "",
        f"输入 B={result['scenario']['batch']}，T={result['scenario']['tokens']}；{result['dimensions']['sublayer_occurrences']} 个 wrapper。",
        "",
        f"前向矩阵 {totals['forward_matrix_flops']:,} FLOPs，反向矩阵 {totals['backward_matrix_flops']:,} FLOPs。",
        "",
        f"含一次 split 的前向标量 {totals['forward_scalar_flops']:,} FLOPs，反向标量 {totals['backward_scalar_flops']:,} FLOPs。",
        "",
        "X 的 residual、pre、linear、RMS 四条梯度支路合并。inner VJP 由调用者提供，真实 Attention/MoE 工作未计入。",
        "",
        "保存边界与有序事件按张量身份列出，X/residual 和最终 comb 不重复计；不声明完整训练内存峰值。",
        "",
        "下表完整列出矩阵、标量、特殊调用、来源、生命周期、保存边界和未知范围。",
        "",
        "| 字段 | 值 |",
        "|---|---|",
    ]
    lines.extend(primitive._complete_fields(result))
    return "\n".join(lines) + "\n"
