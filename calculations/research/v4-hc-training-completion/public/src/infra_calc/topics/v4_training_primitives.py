...
def _complete_fields(value, path=""):
    """Render all recorded leaves, including explicit unknowns and empty values."""
    if isinstance(value, dict) and value:
        for key, child in value.items():
            yield from _complete_fields(child, f"{path}.{key}" if path else key)
    elif isinstance(value, list) and value:
        for i, child in enumerate(value):
            yield from _complete_fields(child, f"{path}[{i}]")
    else:
        rendered = "unknown (null)" if value is None else str(value)
        yield f'| {path} | {rendered.replace(chr(10), "<br>").replace("|", "&#124;")} |'


def markdown(result):
    router = result["router"]
    split = result["sinkhorn_split"]
    lines = [
        "# V4-Flash 训练：Router 与有限 Sinkhorn 子账",
        "",
        f"输入 B={result['scenario']['batch']}，T={result['scenario']['tokens']}。只覆盖声明的局部可微路径。",
        "",
        f"Router 分数矩阵前向 {router['forward_score_matrix_flops']:,} FLOPs，反向 {router['backward_score_matrix_flops']:,} FLOPs。",
        "",
        f"Sinkhorn 保留 {split['normalizations_per_occurrence']} 次有限归一化与 epsilon；反向标量 {split['backward_scalar_flops']:,} FLOPs。",
        "",
    ...
    lines.extend(_complete_fields(result))
    return "\n".join(lines) + "\n"
