...
def calculate(inputs):
    ...
    for source in sources:
        ...
        if (
            ...
            raise ValueError("source integrity mismatch: " + source["file"])
    p = dict(inputs)
    protocol = p["protocol"]
    mode = p["mode"]
...
def scenarios():
    ...
    for protocol in ("tcp_tls13", "quic_v1"):
        for mode in ("fresh", "resume", "early_accept", "early_reject", "reused"):
            rows[protocol + "-" + mode] = example(protocol, mode)
        for suffix, patch in [
            ("wait", {"application_policy": "wait_client_finished"}),
            (
                "reject-no-retry",
                {"mode": "early_reject", "application_retry_authorized": False},
            ),
        ]:
            p = example(protocol, "early_accept")
            p.update(patch)
            rows[protocol + "-" + suffix] = p
    for count, ack in [(3, False), (4, False), (4, True)]:
        p = example()
        p["packets"]["server_flight"] = [1200] * count
        p["server_flight_ack"] = ack
        rows[f"quic-budget-{count * 1200}-ack-{ack}"] = p
    return rows
...
def main():
    ...
    result = (
        ...
        if args.inputs
        else {name: calculate(p) for name, p in scenarios().items()}
    )
