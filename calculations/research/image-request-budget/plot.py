"""Reproducible Fig12-1: declared complete-image budgets, never preview timing."""
...
def build():
    spec = importlib.util.spec_from_file_location('image_budget_candidate', HERE/'calculate.py')
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    archived = json.loads((HERE/'result.json').read_text())
    baseline = module.calculate()
    assert baseline == archived['original']
    ...
    plt.close(fig)
    manifest=dict(figure='12-1',schema_version=1,renderer='Matplotlib Agg',
        python=platform.python_version(),matplotlib=matplotlib.__version__,numpy=np.__version__,
        svg_hashsalt='image-request-budget-figure12-1-v1',svg_date=None,
        input_hashes={name:sha256((HERE/name).read_bytes()).hexdigest() for name in ('calculate.py','result.json','plot.py')},
        artifact_hashes={name:sha256((HERE/name).read_bytes()).hexdigest() for name in ('curve-data.json','figure.png','figure.svg')},
        exact_sample_count=len(points),visual_scope='Additive RTT budget; no observed timing, preview or quality guarantee')
...
if __name__=='__main__':
    ...
    second=build()
    if first!=second:
        raise AssertionError('Regeneration changed figure/data/manifest bytes')
    print(json.dumps(dict(status='reproduced_identically',points=first['exact_sample_count'],hashes=first['artifact_hashes']),indent=2))
