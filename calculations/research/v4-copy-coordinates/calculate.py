...
from infra_calc.sources import model_config,read_source,provenance
...
if __name__=='__main__':
    parser=argparse.ArgumentParser(description=__doc__);parser.add_argument('--rows',type=int,default=32);parser.add_argument('--output',type=Path,required=True)
    args=parser.parse_args();args.output.write_text(json.dumps(calculate(args.rows),indent=2)+'\n')
