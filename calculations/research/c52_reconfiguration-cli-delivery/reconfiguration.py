...
Import infra_calc from the existing checkout (see run_scenarios.py).
"""
...
RUNTIME_TERMS = ('drain', 'group_setup', 'weight_repack', 'state_validation',
...
def layout(config, spec):
    if spec.get('pp', 1) != 1 or spec.get('dp', 1) != 1:
...
def intervals(config, spec, batch, history, auxiliary_bytes):
    ...
    if auxiliary_bytes:
        ranges = partition_ranges(auxiliary_bytes, 1, len(devices), 'flat')
...
def amortization(overhead, saving_per_step):
    ...
    if saving <= 0:
        return dict(status='no_positive_saving', break_even_steps=0 if overhead == 0 and saving == 0 else None,
...
def calculate(scenario):
    ...
    return dict(schema_version=1, calculation='tp-ep-reconfiguration', model=model, scenario=s,
        ...
        summary=dict(network_bytes=network, transfer_lower_bound_exact_seconds=str(bound) if complete_bound else None,
            ...
            declared_serial_switch_seconds_exact=str(bound+runtime_sum) if complete_bound and not missing_runtime else None,
            measured_switch_seconds=None, predicted_end_to_end_seconds=None),
        amortization=dict(time=amortization(a.get('extra_seconds'),a.get('seconds_saved_per_step')),
                          money=amortization(a.get('extra_cost'),a.get('cost_saved_per_step'))),
