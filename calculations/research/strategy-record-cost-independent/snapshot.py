...
def calculate():
    ...
    root = next(parent for parent in folder.parents if (parent / "experiments/ch03/03-03").is_dir())
    sources = json.loads((folder / "sources.lock.json").read_text())
    ...
    return dict(calculation="archived-strategy-record-cost", batches=batches, sources=sources,
