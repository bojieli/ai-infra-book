...
from infra_calc.topics import vision_encoding
from vision_preprocess import calculate, axis_weights
...
class PreprocessTests(unittest.TestCase):
    ...
    def test_hardware_prose_patch_numeric_conservation(self):
        p = Path(__file__).resolve().parent
        patch = json.loads((p / "hardware-prose.patch.json").read_text())
        for row in patch["replacements"]:
            self.assertTrue(
                row["path"] == "/status"
                or row["path"] == "/pending_families"
                or "/notes/" in row["path"]
            )
