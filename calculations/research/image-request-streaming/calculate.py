...
def calculate(chunks=None,mode='whole_image',independent_blocks_authorized=False,
    ...
    return dict(calculation='image-request-streaming-candidate',scenario=dict(chunks=chunks,mode=mode,
...
def build_results():
    preview=dict(after_processed_chunk=0,bytes=500_000,encode_seconds='1/20',quality_contract='declared lower-quality partial preview of processed prefix; not full-image final')
    return {f'{mode}-{suffix}':calculate(mode=mode,independent_blocks_authorized=mode=='independent_blocks',preview=value)
        for mode in ('whole_image','independent_blocks') for suffix,value in (('without-preview',None),('with-preview',preview))}
