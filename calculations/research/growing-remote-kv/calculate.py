...
if __name__=='__main__':
    scenes={}
    for model in ('qwen3-8b','qwen3.6-35b-a3b'):
        for placement in ('remote_all','remote_prefix_local_tail'):
            for copies in (1,2):
                scenes[f'{model}-{placement}-r{copies}']=calculate(model=model,placement=placement,copies=copies)
    Path(__file__).with_name('result.json').write_text(json.dumps(scenes,ensure_ascii=False,indent=2)+'\n')
