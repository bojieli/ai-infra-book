...
- [ ] C68 连接复用／握手/RTT/拥塞窗口、带宽时延积、小请求/大文件/帧截止时间（12.3）。[原要求审计](../research/plan-c68-c69-audit.md)确认已有BDP、声明建链预算与TCP/HTTP3局部实测；[单请求有限ACK窗口](research/connection-window-integration/acceptance.json)已公共接入。现已完成[连续请求公共CLI](src/infra_calc/topics/connection_sequence.py)：同一事件队列/双向FIFO/连接级方向窗口，成片或最后ACK触发下一轮，保留旧ACK和outstanding，4策略/大小请求/第二轮丢段共17场景34产物。118独立trace重放、5新增专项tests、34实际CLI及17完整候选payload核对通过；[公共验收](research/connection-sequence-integration/acceptance.json)：870tests（848通过/22跳过）、2092产物26图、正文网页通过。3官方RFC原件逐调用核验，自定义教学协议不等于真实TCP/QUIC控制器或实测性能。仍缺真实TLS/QUIC新建/恢复/0RTT接受/拒绝/回退、指定实际拥塞控制器、语音/截图/媒体截止与多流业务连接、图12-4；[下一协议范围](research/connection-sequence-integration/next-protocol-scope.md)，保持未勾选。 已新增[协议握手研究候选](research/protocol-handshake/README.md)：TCP+TLS1.3/QUICv1的声明包依赖、early接受/拒绝/应用授权、Initial与三倍预算/Handshake ACK解除；17场景、52独立检查/287发送/21拒绝、6精确手算与实际CLI一致，[候选验收](research/protocol-handshake/acceptance.json)。长early跨密钥切换、HRR/Retry/一般ACK/PTO及公共接入仍待，不把候选当完整协议实现。
