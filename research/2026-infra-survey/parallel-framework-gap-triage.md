...
> 2026-09-09 后续状态：下文保留最初筛选时的证据与待办。随后已补读[磁盘权重更新控制层](root-weight-controller/NOTES.md)、[DSpark 规划与实际输入布局](root-dspark-execution/NOTES.md)、[Ollama MLX 推测执行](parallel-mlx-speculation-body/NOTES.md)。三项均已形成有界结论并用于原有章节，不应根据下文历史措辞重新从头阅读。权重交接仍未验证完整 RL 调用方的故障恢复；DSpark 未覆盖全部 worker／backend 执行；MLX 未做运行测量。它们是保留的结论边界，不是完整系统已验收的声明。
