...
def verify(base=None):
    ...
    for folder in ('parallel-abstracts', 'parallel-abstracts-next', 'parallel-abstracts-third', 'parallel-abstracts-fourth', 'parallel-abstracts-fifth', 'parallel-abstracts-sixth', 'parallel-abstracts-seventh', 'parallel-abstracts-eighth'):
        packet = D / folder
    ...
    summary = {
        ...
        'additional_full_primary_abstracts': 39, 'errors': [],
    }
    summary['remaining_abstracts'] = len(summary['remaining_abstract_orders'])
    assert (summary['primary_abstracts_screened'], summary['public_pdfs'], summary['pdf_pages'], summary['selected_sections_read']) == (80, 70, 1075, 3)
