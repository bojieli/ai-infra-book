...
def verify(base=None):
    ...
    for folder in ('parallel-abstracts', 'parallel-abstracts-next', 'parallel-abstracts-third', 'parallel-abstracts-fourth', 'parallel-abstracts-fifth', 'parallel-abstracts-sixth', 'parallel-abstracts-seventh', 'parallel-abstracts-eighth', 'parallel-abstracts-ninth', 'parallel-abstracts-tenth', 'parallel-abstracts-eleventh'):
        packet = D / folder
    ...
    from verify_vtrain_body import verify as check_vtrain_body
    vtrain = check_vtrain_body()
    entry = entries[12]
    assert entry['abstract_read'] and entry['selected_reading'] is None
    vtrain_path = D / 'vtrain-body-reading/reading.json'
    entry['selected_reading'] = {'proof_file': str(vtrain_path.relative_to(ROOT)),
        'physical_pdf_pages': vtrain['full_text_pages_read'],
        'scope': 'Root read full text p4–7 and visually checked p7 validation; not whole paper.'}
    ...
    summary = {
        ...
        'additional_full_primary_abstracts': sum(e['abstract_read'] for e in entries.values()) - base['primary_abstracts_screened'], 'errors': [],
    }
    summary['remaining_abstracts'] = len(summary['remaining_abstract_orders'])
    assert (summary['primary_abstracts_screened'], summary['public_pdfs'], summary['pdf_pages'], summary['selected_sections_read']) == (90, 76, 1167, 6)
