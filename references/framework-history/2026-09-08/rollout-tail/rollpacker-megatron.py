...
from roll.distributed.strategy.mcore_grad import start_grad_sync, _make_backward_post_hook
...
from roll.distributed.strategy.mcore_sched import get_forward_backward_func
...
@contextmanager
def set_no_sync(models: Any, enabled: bool = True):
    """Context manager to temporarily set the 'no_sync' attribute on a model."""
    _cache_finalize_model_grads_funcs = list()
    for model_id, model in enumerate(models):
        setattr(model, '_enable_no_sync', enabled)
        # print("model.config.no_sync_func  is {}".format(model.config.no_sync_func), flush=True)
        # assert model.config.no_sync_func is None, 'we currently only consider that model.config.no_sync_func is None'
        # finalize_model_grads_func will perform allreduce
        if enabled and eval(os.environ.get('SKIP_FINAL_GRAD', "False")): 
            _cache_finalize_model_grads_funcs.append(model.config.finalize_model_grads_func)
            model.config.finalize_model_grads_func = None
        # setattr(model.config, 'no_sync_func', model.no_sync)
    try:
        yield
    finally:
        for model_id, model in enumerate(models):
            delattr(model, '_enable_no_sync')
            if enabled and eval(os.environ.get('SKIP_FINAL_GRAD', "False")): 
                # pass 
                # model.config.no_sync_func = None 
                model.config.finalize_model_grads_func = _cache_finalize_model_grads_funcs[model_id]
...
class MegatronInferStrategy(InferenceStrategy):
    ...
    def __init__(self, worker: Worker):
        ...
        print(f"[DEBUG MEGATRON INIT] {self.worker_config.use_remove_padding} {self.use_remove_padding} {config_dict}")
    ...
    def initialize(self, model_provider):
        ...
        self.model.config.fixed_gradient_accumulation_steps_in_no_sync = -1 # for no_sync training, set it to -1
    ...
    def get_data_input(self, batch: DataProto):
        ...
        # torch.distributed.get_process_group_ranks(self.intra_distributed_optimizer_instance_group)
    ...
    def forward_step(
        ...
        with disable_gradients(models=self.model.get_models()):
            ...
            losses_reduced: List[Dict[str, torch.Tensor]] = self.forward_backward_func(
                forward_step_func=partial(self.inner_forward_step, forward_func, False),
    ...
    def inner_forward_step(self, loss_func, no_sync: bool, data_iterator: Iterator[DataProto], model):
        # print("no_sync is {}, current step {}".format(no_sync, self.current_step), flush=True)
        self.current_step += 1
        ...
        if self.use_remove_padding:
            cp_size = mpu.get_context_parallel_world_size()
            pad_to_multiple_of = 256 if cp_size == 1 else 2048
            unpad_seq_len = self._get_unpad_seqlen(attention_mask=attention_mask, pad_to_multiple_of=pad_to_multiple_of)
...
class MegatronTrainStrategy(MegatronInferStrategy, TrainStrategy):
    ...
    def initialize(self, model_provider):
        ...
        logger.info(f"{self.model.get_models()}")
        dist.barrier()
        self.fixed_gradient_accumulation_steps_in_no_sync = self.megatron_train_args.gradient_accumulation_steps

    def train_step(self, batch: DataProto, loss_func: Callable, no_sync: bool=False, in_no_sync_train: bool=False, sub_group=None):
        print(f"batch.shape {len(batch.batch)}, rank {dist.get_rank()}", flush=True)
        self.model.train()
        ...
        self.megatron_train_args.gradient_accumulation_steps = num_microbatches
        assert (
        ...
        self.current_step = 0
        print("no_sync is {}, in_no_sync_train {}".format(no_sync, in_no_sync_train), flush=True)
        if in_no_sync_train:
            print("ori_fixed_gradient_accumulation_steps_in_no_sync ".format(no_sync, in_no_sync_train), flush=True)
            ori_fixed_gradient_accumulation_steps_in_no_sync = self.model.config.fixed_gradient_accumulation_steps_in_no_sync
            self.model.config.fixed_gradient_accumulation_steps_in_no_sync = self.fixed_gradient_accumulation_steps_in_no_sync
        # with set_no_sync(self.model.get_models(), no_sync):
        if True:
            metrics_tensors: List[Dict[str, "torch.Tensor"]] = self.forward_backward_func(
                forward_step_func=partial(self.inner_forward_step, loss_func, no_sync),
                data_iterator=data_iterator,
                model=self.model.get_models(),
                num_microbatches=num_microbatches,
                seq_length=self.seq_length,
        ...
        if in_no_sync_train:
            self.model.config.fixed_gradient_accumulation_steps_in_no_sync = ori_fixed_gradient_accumulation_steps_in_no_sync
        ...
        if no_sync:
            ...
            print("no sync mode {}".format(no_sync))
            return metrics

        # 只有step的时候需要load optimizer states
        # if not in_no_sync_train:
        if True: 
        ...
        # print(metrics['actor/weighted_pg_loss'], flush=True)
        return metrics
