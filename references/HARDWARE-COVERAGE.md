...
| Apple：M2 Pro／M2 Max | [Apple GPU 架构说明（WWDC20）](files/documents/apple-gpu-architecture.html)、[Metal 资源模式官方数据](files/documents/apple-metal-memory.json)；[Ollama 固定版本 Metal 缓冲代码](files/documents/ollama-ggml-metal-memory.txt)及[内核](files/documents/ollama-ggml-metal-kernels.txt) | [Apple 2023 年发布资料](files/specs/apple-m2-pro-max.html)；当前实机确认是 M2 Max 38 核 GPU／96 GB，M2 Pro 为参照 | 贯穿第 4、5、8、9 章，比较统一内存、带宽、量化与实际后端。Metal GPU 与 Neural Engine 分开；图形 tile rendering 的材料不证明 LLM 获益；实测尚待完成。 |
| 其他端侧：Apple M5、Snapdragon X Elite | [Apple M5 MacBook Pro 技术规格](files/specs/apple-m5-macbook.html)、[Snapdragon X Elite 产品简介](files/specs/qualcomm-xelite.pdf) | 对应具体整机／SoC 的 CPU、GPU、NPU 与内存参数 | 与已存 Ollama、MLX、llama.cpp、Unsloth 资料结合，不移用 M2 的实验结果。产品参数不能证明某工具实际调用了 NPU；须记录后端、算子回退、持续功耗与设备温度。X Elite 材料为 2023 年产品资料。 |
