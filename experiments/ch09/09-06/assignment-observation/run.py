...
if os.environ.get('BOOK_MOE_OBSERVE_DIR'):
    from observer import install
    install()
async def main(a):
    os.environ['BOOK_MOE_OBSERVE_DIR']=str(a.out.absolute())
    from observer import install
    install()
    ...
    try:
        ...
        with (out/'requests.jsonl').open('w',buffering=1) as f:
            for c in cases:
                (out/'observer-state.json').write_text(json.dumps({'case_id':c['id']}))
        ...
        (out/'observer-state.json').write_text('{}')
