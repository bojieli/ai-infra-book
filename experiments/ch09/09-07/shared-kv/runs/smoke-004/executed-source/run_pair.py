...
def main(args):
    ...
    ctx=mp.get_context('spawn');engines=[];daemon=None;daemonlog=None;records=[]
    def start(name,port):
    ...
    try:
        ...
        command=[str(Path(sys.executable).absolute().parent/'lmcache'),'server',
                 ...
                 '--chunk-size','256','--l1-size-gb','4','--no-l1-use-lazy',
...
if __name__=='__main__':
    ...
    parser.add_argument('--baseline-from',type=Path);main(parser.parse_args())
