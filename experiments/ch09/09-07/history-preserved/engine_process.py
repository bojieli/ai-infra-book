...
def run(connection, output, model, server_port, use_apc):
    ...
    with (root/'engine.log').open('w', buffering=1) as log:
        ...
        try:
            asyncio.run(serve(connection, root, model, server_port, use_apc))
...
async def serve(connection, root, model, server_port, use_apc):
    ...
    config = dict(model=model, dtype='bfloat16', kv_cache_dtype='auto',
                  ...
                  enable_chunked_prefill=True, enable_prefix_caching=use_apc,
    ...
    try:
        ...
        while True:
            ...
            async for item in engine.generate({'prompt_token_ids':req['prompt_token_ids']},
                    SamplingParams(temperature=0, max_tokens=command.get('max_tokens',1200)), req['id']):
