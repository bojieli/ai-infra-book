...
def main():
 ...
 try:
  ...
  order=[];rng=random.Random(911)
  for trial in range(2):
   cases=[dict(trial=trial,policy=p,output_budget=b) for b in [8,32,128] for p in ['cache_first','queue_first','predicted_completion']];rng.shuffle(cases);order.extend(cases)
  (out/'order.json').write_text(json.dumps(order,indent=2)+'\n')
  with concurrent.futures.ThreadPoolExecutor(max_workers=2) as pool:
   for case in order:
    trial=case['trial'];policy=case['policy'];budget=case['output_budget'];label=f'book909c-{trial}-{budget}-{policy}'
    ...
    selected=ports[0] if policy=='cache_first' else min(ports,key=lambda p:state[str(p)][0]['num_reqs']) if policy=='queue_first' else min(ports,key=lambda p:predictions[str(p)])
