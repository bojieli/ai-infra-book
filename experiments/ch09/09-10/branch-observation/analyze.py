...
def main():
    ...
    for c in plan['order']:
        ...
        for e in events:
            ...
            assert e['prefix_input_sha256']==sha(R/'inputs.json')
        ...
        groups.append(dict(**c,group=name,first_completed_rid=first['request_id'],first_api_id=api_id,first_cached_tokens=first['response']['meta_info'].get('cached_tokens'),first_observed_rate_skip_zero_prefill=fc['observed_rate_skip_zero_prefill'],evidence_complete=all(x['evidence_complete'] for x in chains if x['group']==name),installed_pids=sorted(installed),event_pids=sorted({e['pid'] for e in trace}),event_count=len(events),observer_atexit_pids=sorted({e['pid'] for e in events if e['event']=='observer_exit'}),observer_atexit_missing_pids=sorted(installed-{e['pid'] for e in events if e['event']=='observer_exit'}),peak_gpu_MiB=peak,actual_pool_tokens=4096))
    assert len(chains)==18
    assert len({tuple(keys) for group in key_evidence for keys in group['request_hash_keys'].values()})==1
