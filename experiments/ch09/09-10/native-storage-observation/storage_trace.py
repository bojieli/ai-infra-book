...
def install():
    ...
    if dest:
        fd=os.open(dest+'.installed.jsonl',os.O_CREAT|os.O_APPEND|os.O_WRONLY,0o600)
        try:os.write(fd,(json.dumps(dict(pid=os.getpid(),ppid=os.getppid(),monotonic_s=time.monotonic(),event='HiCacheFile observer installed'))+'\n').encode())
        finally:os.close(fd)
