...
metrics=['dram__bytes_op_read.sum','dram__bytes_op_write.sum','lts__t_bytes.sum','gpu__time_duration.sum','launch__registers_per_thread']
records=[]
for m in [1,1024]:
 for mode in ['opaque_custom','compiler_visible','explicit_fusion']:
  label=f'm{m}-{mode}'
  command=['sudo','-n','env','PYTHONPATH='+os.pathsep.join(x for x in sys.path if x),a.ncu,
   ...
   sys.executable,str(ROOT/'counter_target.py'),'--t',str(m),'--path',mode]
...
(a.output/'collection.json').write_text(json.dumps(dict(records=records,metrics=metrics,ncu_version=subprocess.check_output([a.ncu,'--version'],text=True),
 source_hashes={name:hashlib.sha256((ROOT/name).read_bytes()).hexdigest() for name in ['compare.py','counter_target.py','collect_counters.py']},
 scope='One full activation/quantization chain after five warmups, reading sealed comparison inputs. Application replay, normal cache and clock controls. Instrumented kernel times are separate from regular timing.'),indent=2)+'\n')
