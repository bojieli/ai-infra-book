...
def expression(g,u):
 y=(torch.nn.functional.silu(g.float())*u.float()).to(torch.bfloat16).float()
 s=y.abs().amax(-1,keepdim=True).div(448.).clamp_min(1.e-8)
 return (y/s).clamp(-448,448).to(torch.float8_e4m3fn),s

@torch.library.custom_op('book504::opaque',mutates_args=())
def opaque(g:torch.Tensor,u:torch.Tensor)->tuple[torch.Tensor,torch.Tensor]:return expression(g,u)
...
def main(out):
 ...
 for t in [1,1024]:
  ...
  refq,refs=expression(g,u);ref=refq.float()*refs
  records={}
  for name,fn in paths.items():
   ...
   records[name]=dict(first_call_seconds=prepare,fp8_different_elements=int((aq.view(torch.uint8)!=refq.view(torch.uint8)).sum()),
    max_abs_dequant_difference=float((actual-ref).abs().max()),max_scale_difference=float((asc-refs).abs().max()),samples_us=[])
   for _ in range(5):fn()
  for trial in range(11):
   ...
   for name in order:
    ...
    b.record();b.synchronize();records[name]['samples_us'].append(a.elapsed_time(b)*1000/20)
 ...
 contract='FP32 SwiGLU expression -> BF16 rounded intermediate -> FP32 per-row absmax/448 clamped min1e-8 -> E4M3FN. No model weights. Approximate exp may alter rounding; report differences. Warm inputs; 11x20 CUDA-event eager samples include submission gaps.'),indent=2)+'\n')
