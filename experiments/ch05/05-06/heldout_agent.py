...
selection=json.loads((a.search/'summary.json').read_text());assert selection['status']=='selected'
candidate=a.search/f"candidate-{selection['selected_turn']}.py";assert hashlib.sha256(candidate.read_bytes()).hexdigest()==selection['selected_sha256'];validate_code(candidate.read_text())
spec=importlib.util.spec_from_file_location('agent_candidate',candidate);module=importlib.util.module_from_spec(spec);spec.loader.exec_module(module)
...
for label,cpu in cases.items():
 x=cpu.cuda();ref=native(x);y=module.run(x)
...
a.output.write_text(json.dumps(dict(selection=selection,rows=rows,protocol_sha256=hashlib.sha256((ROOT/'protocol.json').read_bytes()).hexdigest(),source_sha256=hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),selection_sha256=hashlib.sha256((a.search/'summary.json').read_bytes()).hexdigest()),indent=2)+'\n')
