...
ROOT=Path(__file__).resolve().parent;out=ROOT/'results/schedule-search-v2'
...
for index,(kind,block,warps) in enumerate(configs):
 ...
 file=out/f'{index}-{kind}.json';command=[sys.executable,str(ROOT/'evaluate_v2.py'),'--kind',kind,'--block',str(block),'--warps',str(warps),'--output',str(file)]
 ...
 if kind in ['native','compiled'] and d['status']!='passed':
  records.append(dict(index=index,kind=kind,file=file.name,status=d['status'],preflight_failed=True));break
