"""Mac own-session resource watchdog, preserving both smoke and formal outputs."""
...
env=os.environ.copy();env.update(OMP_NUM_THREADS='2',OPENBLAS_NUM_THREADS='2',VECLIB_MAXIMUM_THREADS='2',GLOO_SOCKET_IFNAME='lo0',PYTHONDONTWRITEBYTECODE='1')
...
with (B/(a.name+'.log')).open('x') as log:
 ...
 while child.poll() is None:
  try:
   ...
   if rss>8*1024**3 or time.time()-start>600:reason='own process-tree resource/deadline guard';break
