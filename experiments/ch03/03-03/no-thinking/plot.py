...
axes[1].bar(policies,[s[p]['wall_s'] for p in policies],color=colors);axes[1].set_ylabel('Sum of task wall times (seconds)');axes[1].set_title('Observed time; concurrent process present')
...
axes[2].set_xticks(range(3),policies);axes[2].set_ylabel('Generated tokens per candidate');axes[2].axhline(4096,color='gray',ls='--');axes[2].set_title('Thinking disabled, total cap 4096')
for ax in axes:ax.grid(axis='y',alpha=.2);ax.set_axisbelow(True)
fig.suptitle('All strict-format checks failed; timings are not an isolated strategy comparison',fontsize=10);fig.tight_layout();fig.savefig(R/'comparison.png',dpi=160);fig.savefig(R/'comparison.svg')
