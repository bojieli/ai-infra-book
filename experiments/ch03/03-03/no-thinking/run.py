...
async def main(model):
 ...
 (out/'order.json').write_text(json.dumps(order,indent=2));tok=AutoTokenizer.from_pretrained(model,local_files_only=True);(out/'tokenizer-markers.json').write_text(json.dumps({s:tok.convert_tokens_to_ids(s) for s in ['<think>','</think>','<|im_end|>']},indent=2));engine=AsyncLLMEngine.from_engine_args(AsyncEngineArgs(**config))
 async def generate(task,trial,index,group):
  messages=[dict(role='user',content=prompt(task))];ids=tok.apply_chat_template(messages,tokenize=True,return_dict=False,add_generation_prompt=True,enable_thinking=False)
  start=time.monotonic();events=[];final=None
  async for o in engine.generate({'prompt_token_ids':ids},SamplingParams(temperature=.6,top_p=.95,max_tokens=4096,seed=30300+trial*10+index),f'{group}-{index}'):
  ...
  try:
   parsed=json.loads(o.text.strip());assert set(parsed)=={'count'} and type(parsed['count']) is int
   value=parsed['count']
