...
(R/'environment.json').write_text(json.dumps(dict(platform=platform.platform(),device=mx.device_info(),versions={n:importlib.metadata.version(n) for n in ['mlx','mlx-lm','transformers','numpy']},python=platform.python_version(),model=meta,load_start_ns=start,load_end_ns=loaded,load_s=(loaded-start)/1e9,loaded_memory=memory(),run_sha256=hashlib.sha256(Path(__file__).read_bytes()).hexdigest()),indent=2)+'\n')
...
with (R/'short.jsonl').open('x') as f:
 for task in tasks:
  cache=make_prompt_cache(model);row=generate(task,allprompts[task['id']]['short'],cache);f.write(json.dumps(row)+'\n');f.flush();del cache
plan=[(trial,slots) for trial in range(2) for slots in [1,4]];random.Random(807).shuffle(plan)
(R/'plan.json').write_text(json.dumps(plan)+'\n')
for trial,slots in plan:
 mx.clear_cache();mx.reset_peak_memory();case=R/f'{trial}-{slots}slots';case.mkdir();caches=[];chunks=[];begin=clock()
 for task in tasks[:slots]:
  c=make_prompt_cache(model);ids=allprompts[task['id']]['long'];assert len(ids)==8192
  for offset in range(0,8191,512):
   part=ids[offset:min(offset+512,8191)];a=clock();logits=model(mx.array([part]),cache=c);mx.eval(logits,[x.state for x in c]);mx.synchronize();b=clock();chunks.append(dict(task=task['id'],offset=offset,count=len(part),start_ns=a,end_ns=b))
  caches.append(c)
 ready=clock();before=dict(memory=memory(),caches=[describe(c) for c in caches]);rows=[]
 (case/'ready.json').write_text(json.dumps(dict(begin_ns=begin,ready_ns=ready,chunks=chunks,**before),indent=2)+'\n')
 for task,c in zip(tasks[:slots],caches):
  rows.append(generate(task,allprompts[task['id']]['long'][-1:],c))
 (case/'outputs.json').write_text(json.dumps(rows,indent=2)+'\n');(case/'after.json').write_text(json.dumps(memory())+'\n')
...
(R/'completion.json').write_text(json.dumps(dict(completed=True,short=4,long=10))+'\n')
