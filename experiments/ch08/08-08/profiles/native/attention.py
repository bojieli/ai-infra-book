...
class Attention(nn.Module, AttentionLayerBase):
    ...
    def __init__(
        ...
        **extra_impl_args,
        ...
        # METRONOME in-engine windowed KV: force a sliding window on DECODER (text) layers so vLLM
        # builds a SlidingWindowSpec -> the KV manager FREES blocks beyond the window (bounded stored
        # KV) AND the attention backend bounds per-frame compute, for resident streaming requests --
        # no app-level request recycling / re-encode. Opt-in via env (window in tokens). Audio/vision
        # encoders use MMEncoderAttention (a separate class), so this only affects the text decoder.
        import os as _os
        _swa = int(_os.environ.get("METRONOME_INENGINE_SWA", "0") or "0")
        if _swa > 0 and attn_type == AttentionType.DECODER and sliding_window is None:
            sliding_window = _swa
            import sys as _sys
            _sys.stderr.write(f"[METRONOME-SWA] forced sliding_window={sliding_window} "
                              f"prefix={prefix} attn_type={attn_type}\n"); _sys.stderr.flush()
            # METRONOME sink retention: the sink-augmented window mask
            # ([0, sink) ∪ [t-W, t]) is implemented only in the Triton unified
            # attention kernel (FA2's window_size cannot express the union),
            # so route these decoder layers to the Triton backend. The
            # multimodal encoders (MMEncoderAttention) resolve their backend
            # separately and are untouched.
            _sink = int(_os.environ.get("METRONOME_SWA_SINK", "0") or "0")
            _force_triton = _os.environ.get("METRONOME_SWA_TRITON", "0") == "1"
            if (_sink > 0 or _force_triton) and attn_backend is None:
                from vllm.v1.attention.backends.triton_attn import (
                    TritonAttentionBackend,
                )
                attn_backend = TritonAttentionBackend
                _sys.stderr.write(f"[METRONOME-SINK] sink={_sink} tokens; decoder "
                                  f"attention routed to TRITON_ATTN ({prefix})\n")
                _sys.stderr.flush()
