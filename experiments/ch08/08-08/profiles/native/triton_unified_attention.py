...
# METRONOME sink retention (StreamingLLM attention sinks inside a post-hoc
# sliding-window mask): with METRONOME_SWA_SINK=<tokens> set, windowed
# attention additionally attends to the first <tokens> absolute positions of
# every sequence. Requires the KV manager to retain the corresponding blocks
# (see SlidingWindowManager). Read once at import; the EngineCore subprocess
# inherits the env. 0 (default) leaves the stock kernel byte-identical.
import os as _os

_METRONOME_SINK_LEN = int(_os.environ.get("METRONOME_SWA_SINK", "0") or "0")
if _METRONOME_SINK_LEN > 0:
    logger.warning(
        "[METRONOME-SINK] unified_attention: retaining %d sink tokens "
        "inside the sliding-window mask",
        _METRONOME_SINK_LEN,
    )
...
def kernel_unified_attention(
    ...
    # METRONOME sink retention: with SLIDING_WINDOW > 0, additionally attend
    # to (and expect resident KV for) the first SINK_LEN absolute positions
    # of every sequence (StreamingLLM attention sinks). 0 disables.
    ...
    loop_lo, loop_hi, max_seq_prefix_len, tile_start = compute_tile_loop_bounds(
        context_len,
        seq_len,
        cur_batch_query_len,
        q_block_local_idx,
    ...
    # METRONOME sink retention: window pruning skips the leading tiles, so
    # when SINK_LEN > 0 prepend the sink tiles [0, min(ceil(SINK/TILE),
    # tile_start)) as extra iterations of the SAME loop, remapping the loop
    # index. The online softmax is order-invariant, so accumulating the sink
    # tiles first is exact. In 3D (segmented decode) mode only segment 0
    # takes the sink tiles — reduce_segments then merges them exactly once.
    # SINK_LEN == 0 keeps the stock loop byte-identical (constexpr branch).
    if SINK_LEN > 0:
        num_sink_tiles = tl.minimum(cdiv_fn(SINK_LEN, TILE_SIZE), tile_start)
        if IS_3D:
            if segm_idx != 0:
                num_sink_tiles = 0
        num_main_tiles = tl.maximum(loop_hi - loop_lo, 0)
        iter_lo = 0
        iter_hi = num_sink_tiles + num_main_tiles
    else:
        num_sink_tiles = 0
        iter_lo = loop_lo
        iter_hi = loop_hi
    ...
    for it in range(iter_lo, iter_hi):
        if SINK_LEN > 0:
            j = tl.where(it < num_sink_tiles, it, it - num_sink_tiles + loop_lo)
        else:
            j = it
        ...
        if SLIDING_WINDOW:
            ...
            keep_v = (context_len + qpos_lo - seq_offset[:, None]) < SLIDING_WINDOW
            if SINK_LEN > 0:
                keep_v = keep_v | (seq_offset[:, None] < SINK_LEN)
            V = tl.where(keep_v, V, 0.0)
        if USE_PER_TOKEN_HEAD_SCALES:
...
def unified_attention(
    ...
    # METRONOME sink retention applies only to plain sliding-window masks
    # (not chunked attention, where the mask branch would ignore it).
    sink_len_val = (
        _METRONOME_SINK_LEN
        if (sliding_window_val > 0 and chunk_lookback == -1)
        else 0
