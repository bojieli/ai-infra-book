...
def check(c,s,r,fixture):
    ...
    if ok:
        ...
        assert len(events(c,'failure'))==1 and r['error']=='ConnectionError: no_response'
...
def analyze(folder):
    ...
    for name,h in env['source_hashes'].items(): assert digest((ROOT/name).read_bytes())==h, name+' changed'
