...
pilot-v1首个2rank/reshard=True已完成3步并保存全部状态，第二组尚未启动worker便EADDRINUSE退出；控制器从临时端口范围选择了会被重新占用的端口。现选择28000起的非临时端口并检查可绑定，保持loopback。

另在主审查中发现保存证据的tensors字典会跨下一步保留旧梯度，干扰峰值解释。新worker每步保存后显式删除证据引用与输入/输出/loss引用，再继续下一步。原pilot源码、日志、trace和3步状态完整归档，不作为修正后内存对比。新批四配置使用相同修正，数据/模型/优化器/阈值不变。
