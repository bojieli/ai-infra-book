"""UNEXECUTED GPU handoff: actual Megatron Core 1F1B, no custom scheduler.
Run only on a separately authorized Linux host with four NVIDIA GPUs.
"""
import os
...
def main():
    ...
    if not torch.cuda.is_available() or not torch.distributed.is_nccl_available():
        raise SystemExit('BLOCKED: CUDA and NCCL required; no CPU/MPS fallback')
    ...
    parallel_state.initialize_model_parallel(tensor_model_parallel_size=1, pipeline_model_parallel_size=4)
    ...
    model = Model(range(rank*9,(rank+1)*9)).cuda()
    ...
    assert schedule.__name__ == 'forward_backward_pipelining_without_interleaving'
    (out/'environment.json').write_text(json.dumps(dict(torch=torch.__version__,cuda=torch.version.cuda,
        ...
        coexistence='All input/target tensors, stage weights, optimizer and autograd states coexist; CPU reference results retained. Dedicated GPU allocation must be recorded by operator.'),indent=2))
    ...
    try:
        ...
        output_ok = rank != 3 or (len(observed)==8 and all(torch.allclose(v,ref_outputs[i],atol=1e-6,rtol=1e-5) for i,v in observed))
