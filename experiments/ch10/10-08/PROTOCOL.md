...
四题固定顺序：1+1→2、7+8→15、12-5→7、3*4→12。用户提示模板：`Compute {expression}. Reply with only the integer answer.`，模型原始chat template，数据不显式提供system消息（模板自身加入默认system文本）。训练集为四题重复两遍，每步四prompt，每prompt两次采样，共每步8回答、2步；不洗牌、不筛样。seed42、temperature1、top_p1、top_k-1、prompt≤128、response≤16、保留EOS。
...
执行语义说明：数据仅提供user消息；固定Qwen chat template自动补入默认system文本 `You are Qwen, created by Alibaba Cloud. You are a helpful assistant.`，原始tokens与导出的prompt保存其全文。optimizer生效默认值为AdamW weight_decay=0.01、betas=[0.9,0.999]、eps=1e-8；零梯度下的衰减不计为非零学习。
