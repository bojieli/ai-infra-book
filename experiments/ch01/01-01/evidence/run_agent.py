...
async def main(a):
    ...
    environment=dict(config=config,thinking=False,temperature=0,max_tokens=1400,
        ...
        protocol_sha256=sha(ROOT/'protocol.json'),source_hashes={f:sha(ROOT/f) for f in ['run_agent.py','evaluate_agent.py','agent_guard.py','schedule.py']})
...
You must author the full candidate kernel code. Imports: import torch, import triton, import triton.language as tl.
...
'''
    ...
    charged=0.;rows=[];origin=time.perf_counter()
    try:
