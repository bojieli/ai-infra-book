...
python run_reasoning.py --model /local/Qwen3-8B/snapshot --output reasoning-results
...
async def main(args):
    ...
    (root/'environment.json').write_text(json.dumps(dict(config=config,torch=torch.__version__,vllm=vllm.__version__,
        ...
        thinking=True,fixture_origin='authored controlled repair task; model chooses actual tool actions',
    ...
    try:
        with (root/'rounds.jsonl').open('w',buffering=1) as log:
            for turn in range(12):
                tokens=tokenizer.apply_chat_template(messages,tokenize=True,return_dict=False,add_generation_prompt=True,enable_thinking=True)
                ...
                try:
                    action_text=text.split('</think>',1)[1].strip() if '</think>' in text else ''
                    action=json.loads(action_text);reply=execute(action,work)
                ...
                row=dict(turn=turn,messages=messages,prompt_token_ids=tokens,output_token_ids=list(final.outputs[0].token_ids),
                    output_text=text,reasoning_end_token_id=tokenizer.convert_tokens_to_ids('</think>'),finish_reason=final.outputs[0].finish_reason,cached_tokens=final.num_cached_tokens,
