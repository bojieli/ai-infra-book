...
'''
    ...
    samples=[];stop_sampling=threading.Event();phase={'name':'control'}
    def sample():
        while not stop_sampling.is_set():
            usage=resource.getrusage(resource.RUSAGE_SELF)
            status=dict(line.split(':',1) for line in Path('/proc/self/status').read_text().splitlines() if ':' in line)
            samples.append(dict(t_s=time.monotonic()-origin,phase=phase['name'],rss_kib=int(status['VmRSS'].split()[0]),cpu_user_s=usage.ru_utime,cpu_system_s=usage.ru_stime))
    ...
    try:
        with (root/'rounds.jsonl').open('w',buffering=1) as log:
            for turn in range(12):
                ...
                phase['name']='model';model_cpu_start=time.process_time()
                ...
                async for out in engine.generate({'prompt_token_ids':tokens},
                    ...
                    final=out
                end=time.monotonic();model_cpu_end=time.process_time();text=final.outputs[0].text
                phase['name']='tool';child_before=resource.getrusage(resource.RUSAGE_CHILDREN)
                ...
                tool_end=time.monotonic();child_after=resource.getrusage(resource.RUSAGE_CHILDREN);phase['name']='control'
                row=dict(turn=turn,messages=messages,prompt_token_ids=tokens,output_token_ids=list(final.outputs[0].token_ids),
                    ...
                    tool_start_s=tool_start-origin,tool_end_s=tool_end-origin,tool_parent_cpu_s=time.process_time()-cpu_start,model_parent_cpu_s=model_cpu_end-model_cpu_start,
                    tool_child_cpu_s=(child_after.ru_utime+child_after.ru_stime)-(child_before.ru_utime+child_before.ru_stime),
        ...
        stop_sampling.set();monitor.join()
        (root/'resource-samples.json').write_text(json.dumps(dict(controller_pid=os.getpid(),samples=samples,sampling_period_s=.05,scope='Controller process only; includes monitoring, tokenizer and engine frontend, excludes live GPU worker RSS/CPU. Child CPU is reaped-tool rusage delta.'),indent=2)+'\n')
        engine.shutdown()
