...
raw=[json.loads(l) for l in (out/'raw.jsonl').read_text().splitlines()];assert len(raw)==2
assert {(r['trial'],r['thinking'],r['budget']) for r in raw}=={(t,k,b) for t in range(2) for k,b in [(False,1000)]}
inputs={};rows=[]
for r in raw:
 key=r['thinking']
...
independent=json.loads((out/'independent-checks.json').read_text())
for f,h in independent['source_hashes'].items():assert hashlib.sha256((R/f).read_bytes()).hexdigest()==h
assert len(independent['records'])==2
for record in independent['records']:
 assert record['returncode']==0
 checks=record['checks'];assert checks['cases']==1013 and checks['passed']==1013-len(checks['failures'])
 assert hashlib.sha256((out/f"case{record['case_index']}"/'intervals.py').read_bytes()).hexdigest()==record['code_sha256']
meta=json.loads((R/'model-metadata.json').read_text())
assert hashlib.sha256((R/'model-config.json').read_bytes()).hexdigest()==meta['json_files']['config.json']['sha256']
report['independent_checks']=[dict(case_index=r['case_index'],**{k:v for k,v in r['checks'].items() if k!='failures'}) for r in independent['records']]
