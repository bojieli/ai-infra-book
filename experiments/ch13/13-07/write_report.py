...
from ledger import forecast,status
B=Path(__file__).resolve().parent;f=forecast();s=status();o=s.get('observation');current='尚无实际观察，结果保持null；原图的观察位置为空。'
if o:
 current=f"实际全批已完成，10个配对比值的中位数为 **{o['metric']:.6f}**；区间门槛{'通过' if o['interval_met'] else '未通过'}，20条恢复路径的联合质量／token门槛{'通过' if o['quality_gate_met'] else '未通过'}。评判为`{o['verdict']}`。执行开始{o['execution_started_utc']}、完成{o['execution_completed_utc']}，均与预测冻结时间分开登记。\n\n数值区间未通过时，即使方向更有利，也不会改成预测成功；单个配对波动不与中位预测混为一谈。实际观察保存在[最新观察]({s['observation_file']})，模型修订决定在[修订决定](revisions/{Path(s['observation_file']).name})。应用事前规则：{o['model_revision_rule']}。原区间、主观概率和null原件没有重写。"
if o:
 summary=json.loads((B/o['run']/'summary.json').read_text());rows=summary['results'];run_completion=json.loads((B/o['run']/'completion.json').read_text())
 current+=f"\n\n实际{summary['paths']}路径、{sum(len(r['workers']) for r in rows)}个worker、{sum(r['preempted'] for r in rows)}次抢占完成；{summary['checks']}项机制检查通过，{summary['quality_passed']}路径质量通过，{summary['exact_baseline_paths']}路径token与各自baseline一致。真实采样{sum(r['sampled_tokens'] for r in rows)}个token（含EOS），唯一提交{sum(r['unique_tokens'] for r in rows)}个位置，重复投递{sum(r['duplicate_deliveries'] for r in rows)}次。总墙钟{run_completion['wall_s']:.3f}秒，进程组RSS采样峰{max(r['peak_rss_bytes'] for r in rows):,} bytes。没有因不利墙钟剔除任何轮次，也没有启动失败需要清理。"
 current+='\n\n| trial | task | restart秒 | preserve秒 | 配对比值 |\n|---:|---|---:|---:|---:|\n'+'\n'.join(f"|{p['trial']}|{p['task']}|{p['restart_wall_s']:.3f}|{p['preserve_wall_s']:.3f}|{p['ratio']:.6f}|" for p in o['pairs'])
text=f'''# 13-7：一项先冻结再复核的有条件预测
...
[prediction.json](prediction.json)于 **{f['created_utc']}** 冻结；[prediction.sha256](prediction.sha256)绑定不可覆盖的原件，observation和revision仍为null。到期时间为 **2026-09-16 23:59:59（新加坡）**，即UTC15:59:59。到期是最迟复核时间，不要求等待到当天才能新增证据。
...
已封存的11-4 sequence／K96中，保留前缀窗口12.800秒，从头重做11.619秒，实际比值 **{f['strongest_known_counterexample']['ratio']:.6f}**；省去了重复采样，却没有更快。完整历史摘要冻结在[known-evidence/11-4-summary.json](known-evidence/11-4-summary.json)，来源SHA列入原预测，和K192新观察分开。
