...
def processes(token):
    ...
    for p in Path('/proc').iterdir():
        ...
        try:
            if ('BOOK_RETRIEVAL_TOKEN='+token).encode() not in (p/'environ').read_bytes().split(b'\0'):continue
            stat=(p/'stat').read_text().rsplit(')',1)[1].split()
            status=(p/'status').read_text().splitlines()
            rss=next((int(x.split()[1])*1024 for x in status if x.startswith('VmRSS:')),0)
            found[int(p.name)]={'birth':int(stat[19]),'rss':rss,'state':stat[0]}
...
token=uuid.uuid4().hex;env=dict(os.environ,BOOK_RETRIEVAL_TOKEN=token,OMP_NUM_THREADS='4',MKL_NUM_THREADS='4',OPENBLAS_NUM_THREADS='4')
...
(a.out/'launch.json').write_text(json.dumps(dict(command=a.command,pid=proc.pid,token=token,start_s=start,
    timeout_s=3600,own_rss_limit_bytes=40*1024**3,own_gpu_limit_mib=48*1024,global_available_min_bytes=24*1024**3),indent=2)+'\n')
try:
    while proc.poll() is None:
        ...
        elif own_gpu>48*1024:reason='own_gpu_limit'
