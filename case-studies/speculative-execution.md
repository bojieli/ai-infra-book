...
第 9.3 节沿同一问题比较框架变化：每轮草稿和验证花了多少时间，最后产生多少有效 token。Qwen3-8B 负责逐步计算，V4 的 DSpark 和 Ollama 的 Gemma 4／MLX MTP 提供不同执行路径。这里是来源与扩写笔记，没有运行模型或执行下载的代码。
...
实验 9-5 增加固定长度、按 batch 选长度、受支持的按请求预算三种对照。保存实际草稿与验证行数、图档位、KV／状态回滚、主机间隙和逐轮产出，分开算吞吐与用户逐 token 等待。实验 9-6 继续看 Agent 完成时间；图 9-4 把请求预算、实际矩阵行数和产出放在同一张自绘 SVG 上。
