...
第 10.5.1 的框架选择也沿这个顺序讲：先固定同一模型、每题生成数、长度和参数更新范围，再画生成、验证、更新与权重交接。用 Unsloth 的 LoRA／QLoRA 路径对照全参数状态预算，用 TRL 的 vLLM server／colocate 对照独立设备和共享显存，再用 verl 区分训练后端与生成后端。最后才用 AReaL／slime 讨论阶段执行和样本版本。各自的[官方依据与版本边界](../references/framework-history/2026-09-09/rl-framework-selection/NOTES.md)已归档；这些是不同的选择维度，不能排成固定的规模梯队。
